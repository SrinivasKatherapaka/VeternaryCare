-- ============================================================================
-- VETERINARY COMPLIANCE EVIDENCE & RISK CONTROL WORKSPACE
-- Production SQL Schema for Supabase PostgreSQL
-- ============================================================================
...
-- DROP EXISTING TABLES AND TYPES FOR CLEAN DEPLOYMENT IF RE-DEPLOYING
...
-- 3. ANIMALS & PET OWNERS (Clinical Context)
CREATE TABLE pet_owners (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID REFERENCES hospital_tenants(id),
    full_name VARCHAR(255) NOT NULL,
...
-- 6. DECISION & OVERRIDE HISTORY (Immutable Audit)
...
-- Tenant Isolation Policy Example for Evidence
DROP POLICY IF EXISTS tenant_evidence_isolation ON evidence_artifacts;
CREATE POLICY tenant_evidence_isolation ON evidence_artifacts
    FOR ALL
    USING (tenant_id = (SELECT tenant_id FROM app_users WHERE id = auth.uid()));

-- Role-Based Read/Write Restrictions for Decision Logs
DROP POLICY IF EXISTS auditor_read_only_decisions ON decision_audit_logs;
